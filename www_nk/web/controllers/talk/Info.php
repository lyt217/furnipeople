...
{	
	public function __construct()
	{
		...
		$this->load->model('web/talkinfo_model');
	}	    
    ...
    public function search($talk_kind = 'all', $srh_mode = 'no', $start_page = null)
	{
		...
		if ($this->input->post('search_keword')) {    
			...
			redirect($this->config->item('base_url') . 'web/talk/info/search/' . $talk_kind . '/srh');
		} 
		else {
			$paginate_url = '/web/talk/info/search/' . $talk_kind . '/' . $srh_mode;
		}
				
		if ($srh_mode == 'srh') {	
			$data['articleList'] = $this->talkinfo_model->get_article_search($start_page, $limit_page, $talk_kind);
			$total_rows = $this->talkinfo_model->get_articlesearch_rowcount($talk_kind);
			$data['numbering'] = $total_rows - $start_page;
		} 
		else {
			$data['articleList'] = $this->talkinfo_model->get_article_list($start_page, $limit_page, $talk_kind);
			$total_rows = $this->talkinfo_model->get_article_rowcount($talk_kind);
			$data['numbering'] = $total_rows - $start_page;
		}		
		...
		$this->load->view('talk/info_search', $data);
	}
	...
	public function article($idx = null, $return_sep = null)
	{
		...
		$data['articleOne'] = $this->talkinfo_model->get_article_one($idx);
		if (empty($idx) || empty($data['articleOne']))
			show_404();
		
		if ($data['articleOne']->state != 'O')
			redirect($this->config->item('base_url') . 'web/talk/info/search/all/no');
		 
		$this->talkinfo_model->set_readcount_increase($idx);
		$this->_recentlist_setcookie($idx, $data['articleOne']->title);
				
    	if ($return_sep == 'rtnlist')
			$data['return_jscript'] = 'javascript:window.location.href=\'/web/talk/info/search/all/no\';';
		else
    		$data['return_jscript'] = 'javascript:history.back();return false;';
		
		$this->load->view('talk/info_oneview', $data);
	}
    ...
    {		
	    ...
	    if ($this->form_validation->run() === FALSE) {
	        $this->load->view('talk/info_create', $data);
	    } 
	    else {
	    	if ( ! $this->session->has_userdata('user_id')) {	    	
			...
			$affected_rowcnt = $this->talkinfo_model->set_article($this->session->userdata('user_id'));
    ...
    public function update($idx = null, $mode = null)
    {
    	...
    	if ($this->session->has_userdata('user_id')) {
    		if ($data['articleOne']->userdbtable != $this->session->userdata('group_table') || $data['articleOne']->user_id != $this->session->userdata('user_id'))
				redirect($this->config->item('base_url').'web/talk/info/article/'.$idx);
		} 
		else {
	    ...
	    if ($this->form_validation->run() === FALSE) {
			...
			$affected_rowcnt = $this->talkinfo_model->set_article_update($idx, $this->session->userdata('user_id'), $data['articleOne']->kindcode);
			if ($affected_rowcnt == 1)
				redirect($this->config->item('base_url').'web/talk/info/article/'.$idx.'/rtnlist');
			else
				show_404();
	    }
    }
    ...
    public function recomm($idx = null)
    {
        ...
        $articleOne = $this->talkinfo_model->get_article_one($idx);                       
        ...
        else if ($this->talkinfo_model->has_recomm_being($idx)) {
            $return_result['state'] = 'fail';            
            $return_result['msg'] = '이미 추천 하셨습니다.';
        }        
        else {            
            $affected_rowcnt = $this->talkinfo_model->set_recomm($idx);
            if ($affected_rowcnt == 1) {
	...
	{		
		$articleOne = $this->talkinfo_model->has_article_auth($idx, $this->input->post('guest_email'), $this->input->post('guest_passwd'));
		if (empty($articleOne)) {				
			$this->session->set_userdata('deny_msg', '이 비회원 글에 대한 비밀번호 또는 이메일이 일치하지 않습니다!');
			$this->session->mark_as_flash('deny_msg');
			redirect($this->config->item('base_url').'web/talk/info/update/'.$idx, 'location', 302);
		}
	}
    ...
    {
        if ($this->session->has_userdata('user_id')) {            
            $affected_rowcnt = $this->talkinfo_model->set_article_delete($idx, $this->session->userdata('user_id'), $old_kindcode);
        }
        else {
            $this->_nonmember_auth($idx);
            $affected_rowcnt = $this->talkinfo_model->set_article_delete($idx, 0, $old_kindcode);
        }             
    ...
    {        
        ...
        $this->talkinfo_model->set_mypoint($adjust);
    }
