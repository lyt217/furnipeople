...
class Talkinfo_model extends CI_Model
{
	...
	public function get_article_list($start_page, $limit_page, $talk_kind)
	{
		...
		if ($talk_kind == 'all') {
			$sql = "SELECT * FROM talkinfo WHERE state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
			$query = $this->db->query($sql);
		}
		else {
			$sql = "SELECT * FROM talkinfo WHERE kindcode = ? AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
	...
	public function get_article_search($start_page, $limit_page, $talk_kind)
	{
		...
		if ($talk_kind == 'all') {
			//$sql = "SELECT * FROM talkinfo WHERE {$this->session->userdata('search_field')} LIKE '%' ? '%' AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
			$sql = "SELECT * FROM talkinfo WHERE {$this->session->userdata('search_field')} LIKE ? AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
			$query = $this->db->query($sql, array($like_keyword));
		}
		else {
			$sql = "SELECT * FROM talkinfo WHERE {$this->session->userdata('search_field')} LIKE ? AND kindcode = ? AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
	...
	public function set_article($user_id = null)
	{
		...
		$this->db->insert('talkinfo', $data);

		if ($this->db->affected_rows() == 1) {
			$sql = "UPDATE state_rowcnt SET total = total+1, {$this->input->post('kind_code')} = {$this->input->post('kind_code')}+1 WHERE identifier = 'talkinfo'";
			$this->db->query($sql);
	...
	public function set_article_update($article_idx, $user_id = null, $old_kindcode)
	{
		...
		$this->db->update('talkinfo', $data);

		if ($this->db->affected_rows() == 1 && $old_kindcode != $this->input->post('kind_code')) {
			$sql = "UPDATE state_rowcnt SET {$this->input->post('kind_code')} = {$this->input->post('kind_code')}+1, {$old_kindcode} = {$old_kindcode}-1 WHERE identifier = 'talkinfo'";
			$this->db->query($sql);
			return $this->db->affected_rows();
	...
	{
		$query = $this->db->get_where('talkinfo', array('id' => $idx), 1);
		return $query->row();
	}
	...
	{
		$query = $this->db->get_where('talkinfo', array('id' => $idx, 'user_id' => 0, 'guestemail' => $guest_email, 'guestpasswd' => $guest_passwd), 1);
		return $query->row();
	}
    ...
    public function set_article_delete($article_idx, $user_id = null, $old_kindcode)
    {
        ...
        if ($this->db->affected_rows() == 1) {
            $sql = "UPDATE state_rowcnt SET total = total-1, {$old_kindcode} = {$old_kindcode}-1 WHERE identifier = 'talkinfo'";
            $this->db->query($sql);
            return $this->db->affected_rows();
        } else {
            return 0;
    ...
    public function set_mypoint($adjust)
    {
        if ($adjust == 'inc') {
            $data = array(
                    ...
                    'breakdown' => 'Furni Info에서 게시글 등록으로 100포인트 적립',
                    'givepoint' => 100,
                    'givewhere' => 'Furni Info',
                    'giveuser' => '자동',
                    'srhdate' => date('Ymd'),
                    'regdate' => date('Y-m-d H:i:s')
                );
            ...
            $data = array(
                    'usrcode' => $this->session->userdata('user_code'),
                    'usrname' => $this->session->userdata('user_titlename'),
                    'breakdown' => 'Furni Info에서 게시글 삭제로 100포인트 차감',
                    'givepoint' => -100,
                    'givewhere' => 'Furni Info',
                    'giveuser' => '자동',
                    'srhdate' => date('Ymd'),
    ...
    public function set_recomm($article_idx)
    {
        ...
        if ($this->db->affected_rows() == 1) {
            $sql = "UPDATE talkinfo SET onechucnt = onechucnt+1 WHERE id = {$article_idx}";
            $this->db->query($sql);
            return $this->db->affected_rows();
        } else {
            return 0;
   ...
   /**
    ...
    public function has_recomm_being($article_idx)
    {
        ...
        $this->db->where('talk_board', 'talkinfo');
