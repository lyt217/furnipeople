...
class Ttalk_model extends CI_Model
{
    ...
    * Talk 게시판 변경
    */
    public function move_talk($from_board, $article_idx, $to_board){

      $query = $this->db->get_where($from_board, array('id' => $article_idx), 1);
      if ($query->num_rows() > 0){
            $original_article = $query->row();
            $board = '';
            if($to_board == 'talkfree'){
                $board = 'f';
            }
            else if($to_board == 'talkinfo'){
                $board = 'i';
            }
            else if($to_board == 'talkphoto'){
                $board = 'p';
            }
            else if($to_board == 'talkadvertise'){
                $board = 'a';
            }
            else if($to_board == 'talkreview'){
                $board = 'r';
            }
            else if($to_board == 'talkqna'){
                $board = 'q';
            }
            $data = array(
                  'kindcode' => $original_article->kindcode,
                  'kindpname' => $original_article->kindpname,
                  'title' => $original_article->title,
                  'content' => $original_article->content,
                  'writer' => $original_article->writer,
                  'user_id' => $original_article->user_id,
                  'mentcount' => $original_article->mentcount,
                  'readcount' => $original_article->readcount,
                  'onechucnt' => $original_article->onechucnt,
                  'userdbtable' => $original_article->userdbtable,
                  'guestpasswd' => $original_article->guestpasswd,
                  'guestemail' => $original_article->guestemail,
                  'regip' => $original_article->regip,
                  'mentyn' => $original_article->mentyn,
                  'srhdate' => $original_article->srhdate,
                  'regdate' => $original_article->regdate,
                  'board' => $board
            );

            $this->db->insert($to_board,$data);
            $insert_id = $this->db->insert_id();

            $sql_d = "DELETE FROM {$from_board} WHERE id = {$article_idx}";
            $query_d = $this->db->query($sql_d);

            $from_ment = $from_board."_ment";
            $to_ment = $to_board."_ment";
            $sql2 = "SELECT * FROM {$from_ment} WHERE talkboard_id = {$article_idx}";

            if ($result = $this->db->query($sql2)){
              // $moving_ments = $query->result();

              foreach ($result->result() as $row){
            // your code
                  $data0 = array(
                        'talkboard_id' => $insert_id,
                        'groupid' => $row->groupid,
                        'grouplevel' => $row->grouplevel,
                        'content' => $row->content,
                        'writer' => $row->writer,
                        'user_id' => $row->user_id,
                        'userdbtable' => $row->userdbtable,
                        'guestpasswd' => $row->guestpasswd,
                        'guestemail' => $row->guestemail,
                        'regip' => $row->regip,
                        'srhdate' => $row->srhdate,
                        'regdate' => $row->regdate,
                        'updeldate' => $row->updeldate,
                        'state' => $row->state
                  );
                  $this->db->insert($to_ment, $data0);

                  $sql_d0 = "DELETE FROM {$from_ment} WHERE id = {$row->id}";
