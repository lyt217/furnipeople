...
class Shopfind extends CI_Controller
{
    ...
    public function searchajax($biz_kind = 'all', $srh_mode = 'no', $start_page = null)
    {
        ...
        if ($srh_mode == 'srh') {
            $data['moreshopList'] = $this->findshop_model->get_more_shop_find();
            ...
            $data['moreshopList'] = $this->findshop_model->get_more_shop_find($biz_kind);
        ...
        if($data['moreshopList']){
          foreach ($data['moreshopList'] as $shop):
              $shop_jsdata2 .= "['" . $shop->corpname . "', " . $shop->latitude . ", " . $shop->longitude . ", " . $numbering-- . "," .$shop->id ."], ";
          endforeach;
          $data['moreshop_jsdata'] = substr($shop_jsdata2, 0, -2);
