...
class Talkadvertise_model extends CI_Model 
{
	...
	{		
		...
		if ($talk_kind == 'all') {
			$sql = "SELECT * FROM talkadvertise WHERE state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
			$query = $this->db->query($sql);
		} 
		else {			 
			$sql = "SELECT * FROM talkadvertise WHERE kindcode = ? AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
	...
	{	 	
		...
		if ($talk_kind == 'all') {			
			//$sql = "SELECT * FROM talkadvertise WHERE {$this->session->userdata('search_field')} LIKE '%' ? '%' AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
			$sql = "SELECT * FROM talkadvertise WHERE {$this->session->userdata('search_field')} LIKE ? AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";									
			$query = $this->db->query($sql, array($like_keyword));
		} 
		else {			 
			$sql = "SELECT * FROM talkadvertise WHERE {$this->session->userdata('search_field')} LIKE ? AND kindcode = ? AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
	...
	public function set_article($user_id = null) 
	{
		...
		$this->db->insert('talkadvertise', $data);
		
		if ($this->db->affected_rows() == 1) {			
			$sql = "UPDATE state_rowcnt SET total = total+1, {$this->input->post('kind_code')} = {$this->input->post('kind_code')}+1 WHERE identifier = 'talkadvertise'";
			$this->db->query($sql);
	...
	public function set_article_update($article_idx, $user_id = null, $old_kindcode) 
	{
		...
		$this->db->update('talkadvertise', $data);
		
		if ($this->db->affected_rows() == 1 && $old_kindcode != $this->input->post('kind_code')) {			
			$sql = "UPDATE state_rowcnt SET {$this->input->post('kind_code')} = {$this->input->post('kind_code')}+1, {$old_kindcode} = {$old_kindcode}-1 WHERE identifier = 'talkadvertise'";
			$this->db->query($sql);
			return $this->db->affected_rows();
	...
	{
		$query = $this->db->get_where('talkadvertise', array('id' => $idx), 1);
		return $query->row();
	}
	...
	{
		$query = $this->db->get_where('talkadvertise', array('id' => $idx, 'user_id' => 0, 'guestemail' => $guest_email, 'guestpasswd' => $guest_passwd), 1);
		return $query->row();
	}
    ...
    public function set_article_delete($article_idx, $user_id = null, $old_kindcode)
    {
        ...
        if ($this->db->affected_rows() == 1) {          
            $sql = "UPDATE state_rowcnt SET total = total-1, {$old_kindcode} = {$old_kindcode}-1 WHERE identifier = 'talkadvertise'";
            $this->db->query($sql);
            return $this->db->affected_rows();
        } else {            
            return 0;
    ...
    {        
        ...
        if ($this->session->userdata('group_table') == 'member') {          
            $sql = "UPDATE member SET talkcount = talkcount{$adjust_sym}1 WHERE usrcode = '{$this->session->userdata('user_code')}'";
            $this->db->query($sql);        
        } 
        else if ($this->session->userdata('group_table') == 'provider') {            
            $sql = "UPDATE provider SET talkcount = talkcount{$adjust_sym}1 WHERE usrcode = '{$this->session->userdata('user_code')}'";
            $this->db->query($sql);                        
        }    
    }
    ...
    {                                          
        ...
        if ($this->db->affected_rows() == 1) {          
            $sql = "UPDATE talkadvertise SET onechucnt = onechucnt+1 WHERE id = {$article_idx}";
            $this->db->query($sql);
            return $this->db->affected_rows();
        } else {
            return 0;
   ...
   /**
    ...
    {       
        ...
        $this->db->where('talk_board', 'talkadvertise');
