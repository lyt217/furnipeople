...
class Shop extends CI_Controller
{
    ...
    public function home($provider_id = null, $return_sep = null)
    {
        ...
        for($i = 0 ; $i < sizeof($data['pofolList']) ; $i++){
        // foreach($data['pofolList'] as $portfo){
          $data['pofolList'][$i]['photolist'] = $this->shop_model->get_portfolio_photo($data['pofolList'][$i]['id']);
          // echo $portfo['id'];
        }
    ...
    public function updateportfolio()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
          ...
          $portfolio_id = $this->shop_model->set_portfolio();
          // echo $portfolio_id;
          if($portfolio_id > 0){
            $uploaded_files = $this->_upload_prodimage($portfolio_id);
            $is_uploaded = $this->shop_model->set_portfolio_photo($uploaded_files, $portfolio_id);
    ...
    private function _recentlist_setcookie($idx, $corp_name, $biz_kind, $address)
    {
        ...
        private function _upload_prodimage($product_id)
        {
            ...
            $files = array();
            foreach ($_FILES['portfolio_photo'] as $key => $valarr) {
            ...
            foreach ($files as $file)
            {
                if ($file['name'])
                {
                    ...
                    if ($handle->uploaded)
                    {
                        ...
                        if ($handle->processed)
                        {
                            $uploadedFiles[$seq]['portfolio_id'] = $product_id;
