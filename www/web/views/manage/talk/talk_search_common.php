    ...
    <div id="main-container">
        <div id="main-sidebar" class="sidebar sidebar-inverse">
            ...
            <ul id="mainSideMenu" class="nav nav-list nav-side">
                ...
                <li class="accordion-group">
                    <div class="accordion-heading active"><a href="#talkMenu" data-parent="#mainSideMenu" data-toggle="collapse" class="accordion-toggle"><span class="item-icon aweso-icon-list-alt"></span><i class="chevron fontello-icon-right-open-3"></i> 토 크</a></div>
                    <ul class="accordion-content nav nav-list collapse in" id="talkMenu">
                        <li<?php echo @$menu_active['talkfree']; ?>><a href="/web/manage/ttalk/talkcommon/talkfree"><i class="fontello-icon-right-dir"></i>Furni Talk</a></li>
                        <li<?php echo @$menu_active['talkreview']; ?>><a href="/web/manage/ttalk/talkcommon/talkreview"><i class="fontello-icon-right-dir"></i>Furni Review</a></li>
                        <li<?php echo @$menu_active['talkinfo']; ?>><a href="/web/manage/ttalk/talkcommon/talkinfo"><i class="fontello-icon-right-dir"></i>Furni Info</a></li>
                        <li<?php echo @$menu_active['talkphoto']; ?>><a href="/web/manage/ttalk/talkcommon/talkphoto"><i class="fontello-icon-right-dir"></i>Furni Photos</a></li>
                        <li<?php echo @$menu_active['talkadvertise']; ?>><a href="/web/manage/ttalk/talkcommon/talkadvertise"><i class="fontello-icon-right-dir"></i>Furni Ads</a></li>
            ...
            <div class="span12 grider">
                ...
                <div class="widget widget-simple">
                    <div class="widget-header">
                        <h4><i class="fontello-icon-user opaci75"></i> <?php echo df_board_title($tbl_name); ?> <small>(총<?php echo $total_rows; ?>개)</small></h4>
                        ...
                        <?php if ($this->session->has_userdata('admin_find')) {
                            ...
                            <a href="/web/manage/ttalk/talkcommon/<?php echo $tbl_name; ?>/id-desc/<?php echo $srh_mode; ?>" class="btn btn-small btn-navi">작성순</a>
                            <a href="/web/manage/ttalk/talkcommon/<?php echo $tbl_name; ?>/mentcount-desc/<?php echo $srh_mode; ?>" class="btn btn-small btn-navi">댓글순</a>
                            <a href="/web/manage/ttalk/talkcommon/<?php echo $tbl_name; ?>/readcount-desc/<?php echo $srh_mode; ?>" class="btn btn-small btn-navi">열람순</a>
                            <a href="/web/manage/ttalk/talkcommon/<?php echo $tbl_name; ?>/onechucnt-desc/<?php echo $srh_mode; ?>" class="btn btn-small btn-navi">추천순</a><span class="padding-side5"></span>
                            <select name="srh_field" class="selecttwo">
                                <option value="writer" <?php echo set_select('srh_field', 'nickname', @$srh_field['nickname']); ?>>작성자</option>
                                <option value="title" <?php echo set_select('srh_field', 'emailid', @$srh_field['emailid']); ?>>제목</option>
                            </select>
                            <input name="find_word" class="span2" type="text" value="<?php echo @$find_arr['find_word']; ?>">
                            <span class="input-append"><button class="btn" type="submit">검색</button><button class="btn" type="button" onclick="window.location.href='/web/manage/ttalk/talkcommon/<?php echo $tbl_name; ?>';">리셋</button></span>
                        </form>
                    </div>
                        ...
                        <div class="row-fluid">
                            <div class="table-wrapper">
                                <table class="table boo-table table-condensed table-content table-striped table-hover">
                                    ...
                                    <thead>
                                        <tr>
                                            <th scope="col">닉네임<br>제목</th>
                                            <th scope="col">열람수<br>추천수</th>
                                            <th scope="col">댓글수<br>댓글허용</th>
                                            <th scope="col">작성IP<br>비회원메일</th>
                                            <th scope="col">작성일<br>수정일</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    <?php if ($talkList) {
                                    foreach ($talkList as $talk): ?>
                                        <tr>
                                            <td><?php echo $talk->writer; ?><br><?php echo $talk->title; ?></td>
                                            <td><?php echo $talk->readcount; ?> 회<br><?php echo $talk->onechucnt; ?> 회</td>
                                            <td><?php echo $talk->mentcount; ?> 개<br><?php echo $talk->mentyn; ?></td>
                                            <td><?php echo $talk->regip; ?><br><?php echo $talk->guestemail; ?></td>
                                            <td><?php echo date('y.m.d ag:i', strtotime($talk->regdate)); ?><br><?php echo ($talk->updeldate) ? date('y.m.d ag:i', strtotime($talk->updeldate)) : ''; ?></td>
                                        </tr>
                                        <tr>
                                            <td colspan="2">
                                            ...
                                            <td colspan="3">
                                                <a onclick="javascript:window.open('/web/talk/<?php echo substr($tbl_name, 4); ?>/article/<?php echo $talk->id; ?>/rtnlist');" class="btn btn-mini btn-yellow">글보기</a>
                                                <?php if ($talk->userdbtable == 'member') { ?>
                                                <a onclick="javascript:window.open('/web/manage/mmember/user/<?php echo $talk->user_id; ?>');" class="btn btn-mini btn-yellow">일반회원</a>
                                                <?php } else if ($talk->userdbtable == 'provider') { ?>
                                                <a onclick="javascript:window.open('/web/manage/pprovider/user/<?php echo $talk->user_id; ?>');" class="btn btn-mini btn-yellow">기업회원</a>
                                                <?php } ?>&nbsp;
                                                <a onclick="javascript:tobestCopy(<?php echo $talk->id; ?>, '<?php echo $tbl_name; ?>');" class="btn btn-mini btn-orange">베스트등록</a>
                                                <a onclick="javascript:talkDelete(<?php echo $talk->id; ?>, '<?php echo $tbl_name; ?>');" class="btn btn-mini btn-orange">글삭제</a>
                                                <select id="totable<?php echo $talk->id ?>" name="totable" style="width:90px; height:20px; font-size: 12px; margin-top:3px;">
                                                  <option value="">게시판선택</option>
                                                  <option value="talkfree">Furni Talk</option>
                                                  <option value="talkinfo">Furni Info</option>
                                                  <option value="talkphoto">Furni Photos</option>
                                                  <option value="talkadvertise">Furni Ads</option>
                                                </select><a onclick="javascript:talkMove(<?php echo $talk->id; ?>, '<?php echo $tbl_name; ?>');" class="btn btn-mini btn-orange">게시판변경</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; } ?>
                                    </tbody>
                                ...
                                <div class="widget-footer">
                                    <div class="pagination pagination-btn text-center margin-top10">
                                        ...
                                        <ul class="hidden-phone">
                                            <li><a href="/web/manage/ttalk/talkcommon/<?php echo $tbl_name; ?>" class="btn margin-left10">리셋</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
...
function talkMove(talk_idx, tbl_name){
  var totable = $("#totable"+talk_idx).val();
  if(tbl_name == totable){
    alert("이미 "+totable+"에 있는 게시글입니다.");
  }
  else{
    if(confirm('정말 '+totable+'게시판으로 이동하시겠습니까?')){
      $.ajax({
          url: '/web/manage/ttalk/articlemove/'+ tbl_name + '/' + talk_idx + '/' + totable,
          type: 'delete',
          dataType: 'text',
...
function talkDelete(talk_idx, tbl_name) {
    if (confirm('정말 삭제하시겠습니까?')) {
        $.ajax({
            url: '/web/manage/ttalk/articledelete/' + talk_idx + '/' + tbl_name,
            type: 'delete',
            dataType: 'text',
            success: function (data) {
                //alert(data);
...
function tobestCopy(talk_idx, tbl_name) {
    if (confirm('베스트글로 등록하시겠습니까?')) {
        $.ajax({
            url: '/web/talk/best/tobestajax/' + tbl_name + '/' + talk_idx,
            type: 'put',
            dataType: 'text',
            success: function (data) {
                if (data)
