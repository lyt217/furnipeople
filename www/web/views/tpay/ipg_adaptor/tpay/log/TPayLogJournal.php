...
class TPayLogJournal{
	...
	/**
	 * Create a TPayLogJournal instance.
	 */
	private function TPayLogJournal(){
		
	}
	
	/**
	 * get a Single TPayLogJournal instance.
	 */
	public static function getInstance(){
		if(!isset(TPayLogJournal::$instance)){
			TPayLogJournal::$instance = new TPayLogJournal();
		}
		return TPayLogJournal::$instance;
	}
	 ...
	 * 
	 */
	public function configureMnBankLog4PHP(){
		if(!isset($this->appLogger) || !isset($this->eventLogger)){
			try {
				$currentPath = dirname(__FILE__);
				$currentPathXml = $currentPath.'/tpay_log4php.xml';
				...
				if($doc->load($currentPathXml)){
					...
					$appAppendNodeList = $xpath->query("/log4php:configuration/appender[@name='TPAY_FILE']/param[@name='File']");
