...
class Sservice extends CI_Controller 
{
    public function __construct()
    {
        ...
        $this->load->model('manage/sservice_model');
    ...
    public function search($sort_field = 'id-desc', $srh_mode = 'no', $start_page = null)
    {
        ...
        $paginate_url = '/web/manage/sservice/search/' . $sort_field . '/' . $srh_mode;
        $data['action_url'] = $paginate_url;
        $data['srh_mode'] = $srh_mode;

        //▽ Search DB ───────────────────────────────────
        $data['inqList'] = $this->sservice_model->get_inquiry_list($start_page, $limit_page, $sort_field);
        $data['total_rows'] = $this->sservice_model->get_inquiry_rowcount();
                                        
        //▽ Pagination & View ───────────────────────────────────
        $data['pagination'] = $this->_pagination_init($paginate_url, $data['total_rows'], $limit_page);
        $this->load->view('manage/service/inquiry_search', $data);
    }
