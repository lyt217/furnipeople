...
class Qna extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('web/talkqna_model');
    ...
    public function search($talk_kind = 'all', $srh_mode = 'no', $start_page = null)
    {
        ...
        $talk_code_arr = df_uri_talkcode(null, 'data');
        if ($talk_kind != 'all') {
        ...
        if ($this->input->post('search_keword')) {
            ...
            redirect(base_url('web/talk/qna/search/' . $talk_kind . '/srh'));
        }
        else {
            $paginate_url = '/web/talk/qna/search/' . $talk_kind . '/' . $srh_mode;
        }

        if ($srh_mode == 'srh') {
            $data['articleList'] = $this->talkqna_model->get_article_search($start_page, $limit_page, $talk_kind);
            $total_rows = $this->talkqna_model->get_articlesearch_rowcount($talk_kind);
            $data['numbering'] = $total_rows - $start_page;
        }
        else {
            $data['articleList'] = $this->talkqna_model->get_article_list($start_page, $limit_page, $talk_kind);
            $total_rows = $this->talkqna_model->get_article_rowcount($talk_kind);
            $data['numbering'] = $total_rows - $start_page;
        }

        $data['pagination'] = $this->_pagination_init($paginate_url, $total_rows, $limit_page);
        $this->load->view('talk/qna_search', $data);
    }
    ...
    public function article($idx = null, $return_sep = null)
    {
        ...
        $data['articleOne'] = $this->talkqna_model->get_article_one($idx);
        if (empty($idx) || empty($data['articleOne']))
            show_404();

        $this->talkqna_model->set_readcount_increase($idx);
        $this->_recentlist_setcookie($idx, $data['articleOne']->title);

        if ($return_sep == 'rtnlist')
            $data['return_jscript'] = 'javascript:window.location.href=\'/web/talk/qna/search/all/no\';';
        else
            $data['return_jscript'] = 'javascript:history.back();return false;';

        $this->load->view('talk/qna_oneview', $data);
    }
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('talk/qna_create', $data);
        }
        else {
            if ( ! $this->session->has_userdata('user_id')) {
            ...
            $affected_rowcnt = $this->talkqna_model->set_root_article();
            if ($affected_rowcnt == 1) {
                $this->_mypoint_adjust('inc');
                redirect(base_url('web/talk/qna/search/all/no'));
            } else {
                show_404();
            }
        }
    ...
    public function update($idx = null, $mode = null)
    {
        ...
        if ($this->session->has_userdata('user_id')) {
            if ($data['articleOne']->userdbtable != $this->session->userdata('group_table') || $data['articleOne']->user_id != $this->session->userdata('user_id'))
                redirect(base_url('web/talk/qna/article/'.$idx));
        }
        else {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $affected_rowcnt = $this->talkqna_model->set_article_update($idx, $data['articleOne']->kindcode, $data['articleOne']->mentcount, $data['articleOne']->mentyn);
            if ($affected_rowcnt == 1)
                redirect(base_url('web/talk/qna/article/'.$idx.'/rtnlist'));
            else
                show_404();
        }
    }

    public function answer($idx = null)
    {
        if (empty($idx))
            show_404();
        ...
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('talk/qna_create_answer', $data);
        }
        else {
            $affected_rowcnt = $this->talkqna_model->set_answer_article($data['articleOne']->id);
            if ($affected_rowcnt == 1)
                redirect(base_url('web/talk/qna/article/'.$idx.'/rtnlist'));
            else
                show_404();
    ...
    public function answerdelete($idx = null)
    {
        ...
        $affected_rowcnt = $this->talkqna_model->set_answerarticle_delete($idx);
        if ($affected_rowcnt == 1)
            echo 'success';
    }
    ...
    public function recomm($idx = null)
    {
        ...
        $articleOne = $this->talkqna_model->get_article_one($idx);
        ...
        else if ($this->talkqna_model->has_recomm_being($idx)) {
            $return_result['state'] = 'fail';
            $return_result['msg'] = '이미 추천 하셨습니다.';
        }
        else {
            $affected_rowcnt = $this->talkqna_model->set_recomm($idx);
            if ($affected_rowcnt == 1) {
    ...
    {
        $articleOne = $this->talkqna_model->has_article_auth($idx, $this->input->post('guest_email'), $this->input->post('guest_passwd'));
        if (empty($articleOne)) {
            $this->session->set_userdata('deny_msg', '이 비회원 글에 대한 비밀번호 또는 이메일이 일치하지 않습니다!');
            $this->session->mark_as_flash('deny_msg');
            redirect(base_url('web/talk/qna/update/'.$idx), 'location', 302);
        }
    }
    ...
    {
        if ($this->session->has_userdata('user_id')) {
            $affected_rowcnt = $this->talkqna_model->set_article_delete($idx, $this->session->userdata('user_id'), $old_kindcode);
        }
        else {
            $this->_nonmember_auth($idx);
            $affected_rowcnt = $this->talkqna_model->set_article_delete($idx, 0, $old_kindcode);
        }
    ...
    private function _recentlist_setcookie($idx, $title)
    {
        ...
        if ($this->input->cookie('recent_talks')) {
            ...
            $this_articlecode = $this->router->fetch_class() . '_' . $idx;    //qna_9
            ...
            $article[0]['code'] = $this->router->fetch_class() . '_' . $idx;    //qna_9
    ...
    private function _mypoint_adjust($adjust)
    {
        ...
        $this->talkqna_model->set_mypoint($adjust);
    }
