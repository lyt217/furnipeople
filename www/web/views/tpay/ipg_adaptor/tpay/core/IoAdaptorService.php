...
class IoAdaptorService extends AbstractService{
	...
	/* (non-Javadoc)
	 * @see net.infobank.mnbank.adaptor.message.MessageCreator#createMessage(java.lang.Object)
	 */
		...
		if("S" == $mdto->getParameter(ENC_FLAG)){
			...
			$nonCryptBuffer = array_slice($headerBuffer,0,172);
                        ...
                        if(LogMode::isAppLogable()){
                                ...
                                $logJournal->writeAppLog("None Ecrypted Bytes -> [".implode($nonCryptBuffer)."]");
			}
			...
			$encryptData = BlockEncrypt::getInstance()->encrypt(implode($mergeBuffer));
			 ...
			 if(LogMode::isAppLogable()){
                                $logJournal = TPayLogJournal::getInstance();
                                $logJournal->writeAppLog("Ecrypted Bytes -> [".$encryptData."]");
                        }
			...
			if(LogMode::isAppLogable()){
				$logJournal = TPayLogJournal::getInstance();
				$logJournal->writeAppLog("BodyMessage Bytes -> [".implode($bodyBuffer)."]");
			}
			...
			if(LogMode::isAppLogable()){
				$logJournal = TPayLogJournal::getInstance();
				$logJournal->writeAppLog("HeaderMessage Bytes -> [".implode($headerBuffer)."]");
			}
	...
	/* (non-Javadoc)
	 * @see net.infobank.mnbank.adaptor.io.Sendable#send(java.lang.Object)
	 */
	public function send($msg) {
		//((MnBankIoAdaptorTransport)transport).setSocketFactory(SocketFactory.getDefault());
		$responseBytes = $this->transport->doTrx($msg);
		return $responseBytes;
	}
	
	/* (non-Javadoc)
	 * @see net.infobank.mnbank.adaptor.message.MessageParser#parseMessage(java.lang.Object)
	 */
