        ...
        <div id="main-sidebar" class="sidebar sidebar-inverse">
            ...
            <ul id="mainSideMenu" class="nav nav-list nav-side">
                ...
                <li class="accordion-group">
                    <div class="accordion-heading active"><a href="#memberMenu" data-parent="#mainSideMenu" data-toggle="collapse" class="accordion-toggle"><span class="item-icon aweso-icon-list-alt"></span><i class="chevron fontello-icon-right-open-3"></i> 회 원</a></div>
                    <ul class="accordion-content nav nav-list collapse in" id="memberMenu">
                        <li><a href="/web/manage/mmember/search"><i class="fontello-icon-right-dir"></i>개인 회원</a></li>
                        <li class="active"><a href="javascript:void(0);"><i class="fontello-icon-right-dir"></i>기업 회원</a></li>
            ...
            <div class="span12 grider">
                ...
                <div class="widget widget-simple">
                    <div class="widget-header">
                        <h4><i class="fontello-icon-user opaci75"></i> 기업 회원 <!--small>설명필용시</small--></h4>
                        ...
                        <?php if ($this->session->has_userdata('admin_find')) {
                            ...
                            <a href="/web/manage/pprovider/search/id-desc/<?php echo $srh_mode; ?>" class="btn btn-small btn-navi">가입순</a>
                            <a href="/web/manage/pprovider/search/talkcount-desc/<?php echo $srh_mode; ?>" class="btn btn-small btn-navi">토크등록순</a>
                            <a href="/web/manage/pprovider/search/goodscount-desc/<?php echo $srh_mode; ?>" class="btn btn-small btn-navi">상품등록순</a>
                            <a href="/web/manage/pprovider/search/usrpoint-desc/<?php echo $srh_mode; ?>" class="btn btn-small btn-navi">포인트순</a>
                            <a href="/web/manage/pprovider/search/eggticket-desc/<?php echo $srh_mode; ?>" class="btn btn-small btn-navi">에그순</a><span class="padding-side5"></span>
                            <select name="srh_field" class="selecttwo">
                                <option value="corpname" <?php echo set_select('srh_field', 'corpname', @$srh_field['corpname']); ?>>회사명</option>
                                <option value="nickname" <?php echo set_select('srh_field', 'nickname', @$srh_field['nickname']); ?>>닉네임</option>
                                <option value="emailid" <?php echo set_select('srh_field', 'emailid', @$srh_field['emailid']); ?>>이메일</option>
                                <option value="addressjibun" <?php echo set_select('srh_field', 'addressjibun', @$srh_field['addressjibun']); ?>>주소</option>
                            </select>
                            <input name="find_word" class="span2" type="text" value="<?php echo @$find_arr['find_word']; ?>">
                            <span class="input-append"><button class="btn" type="submit">검색</button><button class="btn" type="button" onclick="window.location.href='/web/manage/pprovider/search';">리셋</button></span>
                        </form>
                    </div>
                        ...
                        <div class="row-fluid">
                            <div class="table-wrapper">
                                <table class="table boo-table table-condensed table-content table-striped table-hover">
                                    ...
                                    <thead>
                                        <tr>
                                            <th scope="col">회사명<br>대표번호<!--차후 휴대폰--></th>
                                            <th scope="col">이메일<br>닉네임</th>
                                            <th scope="col">업종<br>기업형태</th>
                                            <th scope="col">담당자<br>담당연락처</th>
                                            <th scope="col">토크등록수<br>상품등록수</th>
                                            <th scope="col">포인트<br>에그</th>
                                            <th scope="col">로그인일<br>로그인횟수</th>
                                            <th scope="col">가입일<br>정보수정일</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    <?php if ($userList) {
                                    foreach ($userList as $user): ?>
                                        <tr>
                                            <td><?php echo $user->corpname; ?><br><?php echo $user->corpphone; ?></td>
                                            <td><?php echo $user->emailid; ?><br><?php echo $user->nickname; ?></td>
                                            <td><?php echo $user->bizcategory; ?><br><?php echo $user->corpkind; ?></td>
                                            <td><?php echo $user->pointer; ?><br><?php echo $user->pointerphone; ?></td>
                                            <td><?php echo $user->talkcount; ?> 개<br><?php echo $user->goodscount; ?> 개</td>
                                            <td><?php echo $user->usrpoint; ?> 포인트<br><?php echo $user->eggticket; ?> 개</td>
                                            <td><?php echo date('y.m.d ag:i', strtotime($user->logindate)); ?><br><?php echo $user->logincount; ?> 회</td>
                                            <td><?php echo date('y.m.d ag:i', strtotime($user->regdate)); ?><br><?php echo ($user->updeldate) ? date('y.m.d ag:i', strtotime($user->updeldate)) : ''; ?></td>
                                        </tr>
                                        <tr>
                                            <td colspan="4">
                                                <i class="fontello-icon-plus"></i> <?php echo $user->addressjibun; ?> <?php echo $user->addrdetail; ?><br>
                                                <i class="fontello-icon-plus"></i> 검색태그: <?php echo str_replace('/', ', ', $user->hashtag); ?>
                                            </td>
                                            <td colspan="4">
                                                <a onclick="javascript:window.open('/web/user/shop/home/<?php echo $user->id; ?>/close');" class="btn btn-mini btn-yellow">내가게</a>
                                                <?php if ($user->goodscount > 0) { ?><a onclick="javascript:window.open('/web/user/shop/product/<?php echo $user->id; ?>');" class="btn btn-mini btn-yellow">등록상품</a><?php } ?>
                                                  <a onclick="javascript:talkDelete(<?php echo $user->id; ?>);" class="btn btn-mini btn-orange">강제탈퇴</a>

                                                <!--a href="#" class="btn btn-mini btn-green">설정</a>
                                                <a href="#" class="btn btn-mini btn-navi">마감</a-->
                                            </td>
                                        </tr>
                                    <?php endforeach; } ?>
                                    </tbody>
                                ...
                                <div class="widget-footer">
                                    <div class="pagination pagination-btn text-center margin-top10">
                                        <?php echo $pagination; ?>
                                        <ul>
                                            <li><a href="/web/manage/pprovider/search" class="btn margin-left10">리셋</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
...
function talkDelete(member_idx) {
    if (confirm('정말 탈퇴시키시겠습니까?')) {
        $.ajax({
            url: '/web/manage/pprovider/memberdelete/' + member_idx,
            type: 'delete',
            dataType: 'text',
