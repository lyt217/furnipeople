...
class Talkreview_model extends CI_Model
{
	...
	public function get_article_rowcount($talk_kind)
	{
		...
		if ($talk_kind == 'all') {
			...
			$this->db->where('identifier', 'talkreview');
			$query = $this->db->get();
			$row = $query->row_array();
			return $row['total'];
		}
	...
	public function get_article_list($start_page, $limit_page, $talk_kind)
	{
		...
		if ($talk_kind == 'all') {
			$sql = "SELECT * FROM talkreview WHERE state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
			$query = $this->db->query($sql);
		}
		else {
			$sql = "SELECT * FROM talkreview WHERE kindcode = ? AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
	...
	public function get_article_search($start_page, $limit_page, $talk_kind)
	{
		...
		if ($talk_kind == 'all') {
			//$sql = "SELECT * FROM talkreview WHERE {$this->session->userdata('search_field')} LIKE '%' ? '%' AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
			$sql = "SELECT * FROM talkreview WHERE {$this->session->userdata('search_field')} LIKE ? AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
			$query = $this->db->query($sql, array($like_keyword));
		}
		else {
			$sql = "SELECT * FROM talkreview WHERE {$this->session->userdata('search_field')} LIKE ? AND kindcode = ? AND state = 'O' ORDER BY id DESC LIMIT {$start_page}, {$limit_page}";
	...
	public function set_article($user_id = null)
	{
		...
		$this->db->insert('talkreview', $data);

		if ($this->db->affected_rows() == 1) {
			$sql = "UPDATE state_rowcnt SET total = total+1, {$this->input->post('kind_code')} = {$this->input->post('kind_code')}+1 WHERE identifier = 'talkreview'";
			$this->db->query($sql);
	...
	public function set_article_update($article_idx, $user_id = null, $old_kindcode)
	{
		...
		$this->db->update('talkreview', $data);

		if ($this->db->affected_rows() == 1 && $old_kindcode != $this->input->post('kind_code')) {
			$sql = "UPDATE state_rowcnt SET {$this->input->post('kind_code')} = {$this->input->post('kind_code')}+1, {$old_kindcode} = {$old_kindcode}-1 WHERE identifier = 'talkreview'";
			$this->db->query($sql);
			return $this->db->affected_rows();
	...
	{
		$query = $this->db->get_where('talkreview', array('id' => $idx), 1);
		return $query->row();
	}
	...
	{
		$query = $this->db->get_where('talkreview', array('id' => $idx, 'user_id' => 0, 'guestemail' => $guest_email, 'guestpasswd' => $guest_passwd), 1);
		return $query->row();
	}
    ...
    public function set_article_delete($article_idx, $user_id = null, $old_kindcode)
    {
        ...
        if ($this->db->affected_rows() == 1) {
            $sql = "UPDATE state_rowcnt SET total = total-1, {$old_kindcode} = {$old_kindcode}-1 WHERE identifier = 'talkreview'";
            $this->db->query($sql);
            return $this->db->affected_rows();
        } else {
            return 0;
    ...
    public function set_mypoint($adjust)
    {
        if ($adjust == 'inc') {
            $data = array(
                    ...
                    'breakdown' => 'Furni Review에서 게시글 등록으로 100포인트 적립',
                    'givepoint' => 100,
                    'givewhere' => 'Furni Review',
                    'giveuser' => '자동',
                    'srhdate' => date('Ymd'),
                    'regdate' => date('Y-m-d H:i:s')
                );
            ...
            $data = array(
                    'usrcode' => $this->session->userdata('user_code'),
                    'usrname' => $this->session->userdata('user_titlename'),
                    'breakdown' => 'Furni Review에서 게시글 삭제로 100포인트 차감',
                    'givepoint' => -100,
                    'givewhere' => 'Furni Review',
                    'giveuser' => '자동',
                    'srhdate' => date('Ymd'),
        ...
        if ($this->db->affected_rows() == 1 && $this->session->userdata('group_table') == 'member') {
            $sql = "UPDATE member SET usrpoint = usrpoint{$adjust_sym}100, talkcount = talkcount{$adjust_sym}1 WHERE usrcode = '{$this->session->userdata('user_code')}'";
            $this->db->query($sql);
        }
        else if ($this->db->affected_rows() == 1 && $this->session->userdata('group_table') == 'provider') {
            $sql = "UPDATE provider SET usrpoint = usrpoint{$adjust_sym}100, talkcount = talkcount{$adjust_sym}1 WHERE usrcode = '{$this->session->userdata('user_code')}'";
            $this->db->query($sql);
        }
    }
    ...
    public function set_recomm($article_idx)
    {
        ...
        if ($this->db->affected_rows() == 1) {
            $sql = "UPDATE talkreview SET onechucnt = onechucnt+1 WHERE id = {$article_idx}";
            $this->db->query($sql);
            return $this->db->affected_rows();
        } else {
            return 0;
   ...
   /**
    ...
    public function has_recomm_being($article_idx)
    {
        ...
        $this->db->where('talk_board', 'talkreview');
