...
class Pprovider extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->library('session');
        $this->load->model('manage/pprovider_model');
    ...
    public function search($sort_field = 'id-desc', $srh_mode = 'no', $start_page = null)
    {
        ...
        $paginate_url = '/web/manage/pprovider/search/' . $sort_field . '/' . $srh_mode;
        $data['action_url'] = $paginate_url;
        $data['srh_mode'] = $srh_mode;

        //▽ Search DB ───────────────────────────────────
        $data['userList'] = $this->pprovider_model->get_provider_list($start_page, $limit_page, $sort_field);
        $total_rows = $this->pprovider_model->get_provider_rowcount();

        //▽ Pagination & View ───────────────────────────────────
        $data['pagination'] = $this->_pagination_init($paginate_url, $total_rows, $limit_page);
        $this->load->view('manage/user/provider_search', $data);
    }
    ...
    public function user($user_idx, $sort_field = 'id-desc', $srh_mode = 'no')
    {
        ...
        //▽ Get DB ───────────────────────────────────
        $data['userList'] = $this->pprovider_model->get_provider_one($user_idx);
        $data['pagination'] = '';
        $this->load->view('manage/user/provider_search', $data);
    }

        public function memberdelete($user_idx){
          $result = $this->pprovider_model->del_user($user_idx);
